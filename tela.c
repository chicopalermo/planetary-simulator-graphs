...
{
    if (!al_install_keyboard()) cai_fora("problema na inicializa??o do teclado do allegro");
    // cria e inicializa a fila de eventos do teclado
    tela_eventos_teclado = al_create_event_queue();
    if (tela_eventos_teclado == NULL) cai_fora("problema na cria??o da fila de eventos do teclado do allegro");
...
void tela_inicializa_janela(float l, float a, char n[])
{
    ...
    if (janela == NULL) cai_fora("problema na cria??o de janela do allegro");
...
void tela_inicio(int largura, int altura, char nome[])
{
    ...
    if (!al_init()) cai_fora("problema na inicializa??o do allegro");
    if (!al_install_mouse()) cai_fora("problema na inicializa??o do mouse do allegro");
    if (!al_init_primitives_addon()) cai_fora("problema na inicializa??o de addons do allegro");
    al_init_font_addon();
    if (!al_init_ttf_addon()) cai_fora("problema na inicializa??o do addon de fontes ttf");
